...
void Sat2P:: write_clauses_2(vector<pair<unsigned int, unsigned int>>& candidate_edges, int C, ofstream& outdata, RationalNumber test_v) {
    for (unsigned int i = 0; i < pVector.size() - 2; i++) {
        ...
        // c2
        for (int l = i - 1; l >= 0; l--) {
            if ((pVector[j] - pVector[l])/(pVector[j] - pVector[i]) <= test_v) {
        ...
        for (int r = j + 1; r < pVector.size(); r++) {
            if ((pVector[r] - pVector[i])/ (pVector[j] - pVector[i]) <= test_v) {
        ...
        for (int l = i - 1; l >= 0; l--) {
            ...
            for (unsigned int r = j + 1; r < pVector.size(); r++) {
                if (pVector[r] - pVector[l]/ (pVector[j] - pVector[i]) <= test_v) {
                    outdata << C << " ";
                    C++;
...
void Sat2P::write_clauses_3(vector<pair<unsigned int, unsigned int>>& candidate_edges,
    ...
    */
    for (unsigned int i = 0; i < pVector.size() - 3; i++) {
        ...
        int C_start = C;
        for (int l = i; l >= 0; l--) {
            if ((pVector[j] - pVector[l])/ (pVector[j] - pVector[i]) > test_v) break;
            for (unsigned int r = j; r < pVector.size(); r++) {
                if ((pVector[r] - pVector[l])/ (pVector[j] - pVector[i]) <= test_v) {
                    ...
                    candidates_ende_pairs.push_back(
                        pair<unsigned int, unsigned int>(l, r));
                    // for c43
                    if ((pVector[r] - pVector[l] + pVector[i + 2] - pVector[i + 1])/ (pVector[j] - pVector[i]) <=
                        test_v) {
...
void Sat2P::write_solver_input(bool only_short, RationalNumber test_v, int test_len) {
    ...
    if (!outdata) {
        ...
        cerr  << " Error: (write_solver_input) file could not be opened"
            << endl;
        cerr << instance_f << endl;
        exit(1);
    }
...
RationalNumber Sat2P::solve() {
    ...
    //vector<RationalNumber> candidate_ods{RationalNumber(599, 448), RationalNumber(230, 167) };
    //vector<RationalNumber> candidate_ods{ RationalNumber(1224, 889)};
    //vector<RationalNumber> candidate_ods{ RationalNumber(599, 448) };
    ...
    while (r >= l) {
        ...
        instance_f = file_d +  "_DIMACS.txt";
        solution_f = file_d +  "_DIMACS_solution.txt";
